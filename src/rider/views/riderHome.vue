    ...
    <div class="p-2 border border-slate-900/20 rounded-md">
      <div class="relative overflow-x-auto">
        ...
        <table class="min-w-full text-sm text-left text-gray-900 sm:rounded-md">
          <thead class="text-xs text-sky-100 uppercase bg-gradient-to-r from-blue-500 from-10% via-violet-500 via-30% to-orange-500 to-90% sm:rounded-md">
            <tr>
          ...
          <tbody>
            <tr class="bg-gray-200 border-b border-gray-700">
              <td class="px-2 py-2 sm:px-6 sm:py-4">atila</td>
              <td class="px-2 py-2 sm:px-6 sm:py-4">0953496u59634</td>
              <td class="px-2 py-2 sm:px-6 sm:py-4">
                <p class="bg-orange-300 p-1 rounded-full px-2">Ready</p>
              </td>
              <td class="px-2 py-2 sm:px-6 sm:py-4 flex justify-center">
                <button @click="showModal = true" class="bg-blue-500 p-3 rounded hover:bg-blue-600">Details</button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-start justify-center p-4">
        <div class="bg-white rounded-lg p-5 shadow-lg w-full max-w-md">
          ...
          <div class="mt-4 flex justify-between">
            <button @click="showModal = false" class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">Accept</button>
            <button @click="showModal = false" class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600">Close</button>
          </div>
        </div>
      </div>
    </div>
