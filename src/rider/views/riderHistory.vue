<template>
    <div class="p-2 border border-slate-900/20 rounded-md">
      <div class="relative overflow-x-auto">
        <h1 class="pb-3 sticky left-0">History</h1>
      ...
      >
        <div class="bg-white rounded-lg p-5 shadow-lg w-full max-w-md">
          ...
          <ul class="space-y-2" v-for="(detail, index) in details" :key="index">
            ...
            <li><strong>Price:</strong> {{ formatPrice(detail.total_price_products) }}</li>
          </ul>
  ...
  export default {
    ...
    setup() {
      ...
      const getOrders = async () => {
        try {
          const urli = `${url}/Ecommerce/vue-project/src/backend/rider/riderApi.php?action=getOrdersHistory`;
