<template>
  <div v-if="isVisible" class="fixed inset-0 flex items-center justify-center bg-gray-500 border-2 border-zinc-300 bg-opacity-75 z-60">
    <div class="bg-white rounded-lg shadow-xl max-w-lg mx-auto p-6">
      <div class="flex font-sans">
        <div class="flex-none w-48 relative">
          <img :src="'data:image/png;base64,' + product.image" :alt="product.name" class="absolute inset-0 w-full h-auto object-cover" loading="lazy"/>
        </div>
        <form class="flex-auto p-6">
          ...
          <div class="flex space-x-4 mb-6 text-sm font-medium">
            <div class="flex-auto flex space-x-4">
              <button class="h-10 px-6 font-semibold rounded-md border border-black-800 text-gray-900" type="button">
                Add to cart
              </button>
            </div>
